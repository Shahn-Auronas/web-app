/*
 * Republished from Pragmatic Bookshelf - Node.js the Right Way
...
module.exports = function (config, app) {
	...
	app.get('/api/bundle/:id', function (req, res) {
		...
		Q.nfcall(request.get, config.b4db + '/' + req.params.id).then(
			function (args) {
				let couchRes = args[0],
			...
			//force any unhandled rejcted promises to throw
			.done();
	});

	//Set the specificed bundle's name
	...
	app.put('/api/bundle/:id/book/:pgid', function (req, res) {
		...
		Q.nfcall(request.get, config.b4db + '/' + req.params.id).then(
			function (args) {
				...
				//look up the book by its Project Gutenberg ID pgid
				return Q.nfcall(request.get, config.bookdb + '/' + req.params.pgid);
			}).then(
			function (args) {
				couchRes = args[0];
				book = JSON.parse(args[1]);
				//fail fast if we can't retreive the book
				if (couchRes.statusCode !== 200) {
    	...
    	Q.nfcall(request.get, config.b4db + '/' + req.params.id).then(
    		function (args) {
    			...
    			if (!(req.params.pgid in bundle.books)) {
    				res.json(409, {
    					error: "conflic",
    					reaason: "Bundle does not contain that book."
    				});
    				return;
    			}
